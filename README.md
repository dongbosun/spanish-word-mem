...
- 不要求用户输入英文或西语答案
- MVP 不实现选择题翻译测试
- 不强制登录、支付、广告或 OpenAI API
- 默认可离线使用本地进度；可选 Google 登录 + Firebase Firestore 云端同步
...
- 导出和导入本地学习进度
- 使用 Google 登录后在 Firestore 中同步学习进度
...
本地如需测试 Firebase 同步，复制 `.env.example` 为 `.env.local`，并填入 Firebase Web App 配置：
...
不要直接在 UI 中写死 `localStorage`。必须通过存储抽象层访问。本地存储仍是第一层缓存，即使开启 Firebase 同步，也先写入本机再异步同步云端。
...
云端同步是可选功能。未配置 Firebase 环境变量或未登录 Google 时，应用继续使用本地进度。

启用后：

- Google 登录状态由 Firebase Auth 管理
- 进度保存在 Firestore 的 `users/{uid}/progress/current`
- 登录时会合并本地和云端进度，并写回本地与云端
- 每次打分、收藏、重置或导入进度后，会继续更新本地缓存并同步 Firestore

仓库包含 `firestore.rules`，规则只允许当前登录用户读写自己的进度文档：
...
当前实现为了简单，把整个 `ProgressMap` 放在一个 Firestore 文档里。3000 词规模下通常足够；如果未来每个词的进度字段继续膨胀，可能需要改为每个词一个文档。
